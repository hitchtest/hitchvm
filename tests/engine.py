    ...
    def set_up(self):
        ...
        if self.settings.get("boxcache", True):
            if "boxcache" in self.preconditions:
                self.path.engine.joinpath("boxcache").joinpath(
                    self.preconditions['boxcache']
                ).copy(self.path.state)
        ...
        run(self.pip("uninstall", "hitchvm", "-y").ignore_errors())
        ...
        if self.preconditions.get("runservices", True):
            ...
            self.ipython_kernel_filename = \
                self.services['IPython'].wait_and_get_ipykernel_filename()
